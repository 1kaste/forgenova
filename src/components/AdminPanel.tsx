...
const AdminPanel: React.FC = () => {
    ...
    }, [activeTab, context?.content]);

    if (!context) return <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center text-white">Connecting...</div>;
    
    const { 
      content, updateContent, authLevel, login, devLogin, logout, setShowAdminPanel, 
      isDefaultAdminPassword, updateAdminPassword,
      updateMasterPassword, inactivityTimeout, setInactivityTimeout, resetAdminPassword,
    } = context;
    ...
    const modifyContent = (modification: (draft: SiteContent) => void) => {
    ...
    const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>, mode: 'light' | 'dark') => {
        ...
        reader.onloadend = () => {
            modifyContent(draft => {
              draft.logo[mode] = reader.result as string;
            });
        };
    ...
    const handleAddService = () => {
      const newId = (content.services.length > 0 ? Math.max(...content.services.map(s => s.id)) : 0) + 1;
      ...
      modifyContent(draft => { draft.services.push(newService); });
    };
    ...
    const handleAddImageFromUrl = (serviceId: number) => {
        ...
        if (urls.length > 0) { 
            modifyContent(draft => {
                ...
                if (service) service.gallery.push(...urls);
            });
    ...
    const handleImageUpload = (serviceId: number, e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Promise.all(filePromises).then(base64Strings => { 
            modifyContent(draft => {
                const service = draft.services.find(s => s.id === serviceId);
                if(service) service.gallery.push(...base64Strings);
            });
        }).catch(error => { console.error("Error reading files:", error); alert("An error occurred while uploading images."); });
    };

    const handleAddNewTheme = () => {
        const newThemeName = `New Theme ${content.themes.length + 1}`;
        const newTheme: ThemeOptions = { name: newThemeName, light: {}, dark: {} };
        const newIndex = content.themes.length;
        modifyContent(draft => { draft.themes.push(newTheme); });
    ...
    const handleSocialIconUpload = (e: React.ChangeEvent<HTMLInputElement>, id: number) => {
      ...
      reader.onloadend = () => { 
        modifyContent(draft => {
          const social = draft.socials.find(s => s.id === id);
          if (social) social.iconUrl = reader.result as string;
        });
      };
      reader.readAsDataURL(file);
      e.target.value = '';
    ...
    if (!content) return <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center text-white">Loading Editor...</div>;
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-6 bg-background text-foreground">
                    ...
                    <div className="animate-fade-in space-y-8">
                        <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                           ...
                           <div className="space-y-6">
                             <Input label="Company Name" value={content.companyName} onChange={e => modifyContent(draft => { draft.companyName = e.target.value })} />
                             <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        {content.logo.light && <img src={content.logo.light} alt="Light Logo Preview" className="h-20 w-full bg-muted p-2 rounded-md mb-2 object-contain" />}
                                        <Input label="URL" value={content.logo.light} onChange={e => modifyContent(draft => { draft.logo.light = e.target.value })} placeholder="URL or upload"/>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {content.logo.dark && <img src={content.logo.dark} alt="Dark Logo Preview" className="h-20 w-full bg-muted p-2 rounded-md mb-2 object-contain" />}
                                        <Input label="URL" value={content.logo.dark} onChange={e => modifyContent(draft => { draft.logo.dark = e.target.value })} placeholder="URL or upload"/>
                         ...
                         <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                           ...
                           <div className="space-y-6">
                             <Input label="Hero Image URL" value={content.hero.image} onChange={e => modifyContent(draft => { draft.hero.image = e.target.value })} />
                             <Input label="Hero Heading" value={content.hero.heading} onChange={e => modifyContent(draft => { draft.hero.heading = e.target.value })} />
                             <Textarea label="Hero Subheading" value={content.hero.subheading} onChange={e => modifyContent(draft => { draft.hero.subheading = e.target.value })} />
                           </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="animate-fade-in">
                        <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                                {content.themes.filter(t => !t.deletedOn).map((theme, index) => (
                                    <ThemeCard
                                        key={`${index}-${theme.name}`}
                                        theme={theme}
                                        isActive={theme.name === content.activeTheme}
                                        onActivate={() => modifyContent(draft => {draft.activeTheme = theme.name})}
                                        onEdit={() => handleEditTheme(index)}
                                        onDelete={() => handleTrashAction('move', 'theme', { themeIndex: content.themes.findIndex(t => t.name === theme.name) }, `the "${theme.name}" theme`)}
                                        draggable={theme.name !== content.activeTheme}
                                        onDragStart={(e) => handleDragStart(e, 'theme', { themeIndex: content.themes.findIndex(t => t.name === theme.name) })}
                        ...
                        {editingThemeIndex !== null && content.themes[editingThemeIndex] && (
                            <div ref={themeEditorRef} className="mt-8 animate-fade-in">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-2xl font-bold">Editing: <span className="text-accent">{content.themes[editingThemeIndex].name}</span></h3>
                 ...
                 {activeTab === 'contact' && (
                    <div className="animate-fade-in space-y-8">
                        <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <Input label="Phone Number" value={content.contact.phone} onChange={e => modifyContent(draft => { draft.contact.phone = e.target.value })} />
                                    <Input label="Email Address" value={content.contact.email} onChange={e => modifyContent(draft => { draft.contact.email = e.target.value })} />
                                </div>
                                <Input label="Physical Address" value={content.contact.address} onChange={e => modifyContent(draft => { draft.contact.address = e.target.value })} />
                            </div>
                        </div>
                 ...
                 {activeTab === 'footer' && (
                    <div className="animate-fade-in">
                        <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                           ...
                           <Textarea label="About Us Column" value={content.footer.about} onChange={e => modifyContent(draft => { draft.footer.about = e.target.value })} />
                        </div>
                    </div>
                )}
                {activeTab === 'developer' && authLevel === AuthLevel.DEV && (
                    <div className="animate-fade-in space-y-8">
                        <div className="bg-card p-6 rounded-lg border border-border shadow-md">
                            ...
                            <div className="space-y-6">
                               <Input label="Footer Credit Text" value={content.footer.credit} onChange={e => modifyContent(draft => { draft.footer.credit = e.target.value })} />
                               <Input label="Footer Credit Link (URL)" value={content.footer.creditLink || ''} onChange={e => modifyContent(draft => { draft.footer.creditLink = e.target.value })} placeholder="https://example.com"/>
                            </div>
                        </div>
                        <div className="bg-card p-6 rounded-lg border border-red-500/50 shadow-md">
                            ...
                            <div className="space-y-8 divide-y divide-border/50">
                                ...
                                <div className="pt-8">
                                    <div className="space-y-2">
                                       <Input label="Inactivity Timeout (seconds)" type="number" value={inactivityTimeout} onChange={e => setInactivityTimeout(Number(e.target.value))} />
                                    </div>
                                </div>
                                <div className="pt-8">
                                    <div className="space-y-2">
                ...
                <div className="animate-fade-in">
                    ...
                    {content.services.filter(s => !s.deletedOn).map(service => (
                        <div 
                            key={service.id} 
                    ...
                    {trashedItemsCount === 0 ? (
                      ...
                      <div className="space-y-4">
                        {content.services.filter(s => s.deletedOn).map(item => {
                          const elapsedMs = currentTime - (item.deletedOn || 0);
                        ...
                        {content.themes.map((item, index) => item.deletedOn ? (() => {
                          ...
                          const isDeletable = elapsedMs > 30000;
                          const secondsLeft = Math.ceil((30000 - elapsedMs) / 1000);
                          const itemIndexInOriginal = content.themes.findIndex(t => t.name === item.name);
                          return (
                            <div key={`t-${index}`} className="flex items-center justify-between bg-muted p-3 rounded-lg border border-border">
                                ...
                                <div className="flex items-center gap-4">
                                    <button onClick={() => handleTrashAction('restore', 'theme', { themeIndex: itemIndexInOriginal }, '')} className="flex items-center gap-1 text-sm text-green-400 hover:underline"><RestoreIcon className="h-4 w-4"/> Restore</button>
                                    <button onClick={() => handleTrashAction('delete', 'theme', { themeIndex: itemIndexInOriginal }, `the "${item.name}" theme`)} disabled={!isDeletable} className="flex items-center gap-1 text-sm text-red-400 hover:underline disabled:text-gray-500 disabled:no-underline disabled:cursor-wait"><TrashIcon className="h-4 w-4"/> {isDeletable ? 'Delete Forever' : `Wait ${secondsLeft}s`}</button>
                                </div>
                            </div>
                          );
                        })() : null)}
                        ...
                        {content.services.flatMap(service => (service.deletedGallery || []).map((item, index) => {
                          ...
                          const isDeletable = elapsedMs > 30000;
                          const secondsLeft = Math.ceil((30000 - elapsedMs) / 1000);
                          const imageIndexInOriginal = (content.services.find(s => s.id === service.id)?.deletedGallery || []).findIndex(i => i.url === item.url);
                          return (
                            <div key={`g-${service.id}-${index}`} className="flex items-center justify-between bg-muted p-3 rounded-lg border border-border">
                                ...
                                <div className="flex items-center gap-4">
                                    <button onClick={() => handleTrashAction('restore', 'galleryImage', { serviceId: service.id, imageIndex: imageIndexInOriginal }, '')} className="flex items-center gap-1 text-sm text-green-400 hover:underline"><RestoreIcon className="h-4 w-4"/> Restore</button>
                                    <button onClick={() => handleTrashAction('delete', 'galleryImage', { serviceId: service.id, imageIndex: imageIndexInOriginal }, 'this image')} disabled={!isDeletable} className="flex items-center gap-1 text-sm text-red-400 hover:underline disabled:text-gray-500 disabled:no-underline disabled:cursor-wait"><TrashIcon className="h-4 w-4"/> {isDeletable ? 'Delete Forever' : `Wait ${secondsLeft}s`}</button>
                                </div>
                            </div>
                          );
                        }))}
        ...
        >
            ...
            <div className="flex items-center gap-2 mb-4">
                {colorsToShow.map(colorKey => (
                    <div key={colorKey} title={colorKey} className="w-6 h-6 rounded-full border border-border" style={{ backgroundColor: palette[colorKey as keyof typeof palette] || 'transparent' }}></div>
                ))}
            </div>
