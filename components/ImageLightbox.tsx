...
const ImageLightbox: React.FC<ImageLightboxProps> = ({ images, startIndex, onClose }) => {
  ...
  return (
    ...
    >
      <button
        className="absolute top-3 left-3 md:top-4 md:left-6 text-white text-5xl font-light hover:text-accent transition-colors z-10"
        onClick={onClose}
      ...
      <div className="relative w-full h-full flex items-center justify-center" onClick={e => e.stopPropagation()}>
        <button
          className="absolute left-0 md:left-10 text-white p-4 text-4xl hover:bg-white/10 rounded-full transition-all duration-300 z-10"
        ...
        <button
          className="absolute right-0 md:right-10 text-white p-4 text-4xl hover:bg-white/10 rounded-full transition-all duration-300 z-10"
          onClick={goToNext}
