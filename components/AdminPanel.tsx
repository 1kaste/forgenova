...
const RestoreIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h5M20 20v-5h-5m15-11l-3.5 3.5M4 4l16 16" /></svg>
);
const TrashNavIcon = () => (
...
const ThemeCard = ({ theme, isActive, onActivate, onEdit, onDelete, draggable, onDragStart, onDragEnd }: {
    ...
    return (
        <div 
          className={`bg-card p-4 rounded-lg border ${isActive ? 'border-primary ring-2 ring-primary' : 'border-border'} transition-all ${!isActive ? 'cursor-grab' : 'cursor-not-allowed'}`}
          draggable={draggable}
          onDragStart={onDragStart}
          onDragEnd={onDragEnd}
        >
            <div className="flex justify-between items-center mb-3">
                <h4 className="text-lg font-bold text-card-foreground">{theme.name}</h4>
            ...
            <div className="flex justify-end gap-2 items-center">
                {!isActive && <button onClick={onDelete} className="text-red-400 hover:text-red-600 hover:underline text-sm font-medium flex items-center gap-1" aria-label={`Move ${theme.name} theme to trash`}><TrashIcon className="h-4 w-4"/>Delete</button>}
...
const AdminPanel: React.FC = () => {
    ...
    }, [context?.content, context?.inactivityTimeout]);
    ...
    useEffect(() => {
        if (activeTab === 'services' && newlyAddedServiceId.current !== null) {
            const el = document.getElementById(`service-admin-card-${newlyAddedServiceId.current}`);
            if (el) {
                setTimeout(() => {
    ...
    useEffect(() => {
        if (activeTab === 'contact' && newlyAddedSocialId.current !== null) {
            const el = document.getElementById(`social-admin-card-${newlyAddedSocialId.current}`);
            if (el) {
                setTimeout(() => {
                    el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    el.classList.add('new-item-highlight');
    ...
    const { 
      ...
      inactivityTimeout, setInactivityTimeout, resetAdminPassword,
    ...
    const handleSave = () => {
        if(editedContent && isDirty) {
          updateContent(JSON.parse(JSON.stringify(editedContent))); // deep copy
          if (newTimeout !== inactivityTimeout) {
    ...
    const handleAddSocialLink = () => {
        setEditedContent(prev => {
            if (!prev) return null;
            const newId = prev.socials.length > 0 ? Math.max(...prev.socials.map(s => s.id)) + 1 : 1;
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (window.confirm(`Are you sure you want to move ${itemName || 'this item'} to the trash? This is a temporary action and can be undone from the trash.`)) {
            moveItemToTrash(data.itemType, data);
        }
    ...
    const trashedItemsCount = [
        content.services.filter(s => s.deletedOn).length,
        content.themes.filter(t => t.deletedOn).length,
        content.socials.filter(s => s.deletedOn).length,
        ...content.services.map(s => (s.deletedGallery || []).length)
    ].reduce((a, b) => a + b, 0);
        ...
        <div className="relative bg-[#0F172A] text-white w-full max-w-7xl mx-auto rounded-lg shadow-2xl flex flex-col overflow-hidden">
          <header className="bg-secondary p-4 flex justify-between items-center flex-shrink-0 border-b border-border">
            ...
            <div>
             ...
             <button onClick={handleClose} className="text-muted-foreground hover:text-white">&times; Close</button>
            </div>
          </header>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                                {editedContent.themes.map((theme, index) => !theme.deletedOn && (
                                    <ThemeCard
                                        ...
                                        onDelete={() => {
                                            if (window.confirm(`Are you sure you want to move the "${theme.name}" theme to the trash? This is a temporary action and can be undone from the trash.`)) {
                                                moveItemToTrash('theme', { themeIndex: index });
                                            }
                                        }}
                        ...
                        {editingThemeIndex !== null && editedContent.themes[editingThemeIndex] && (
                            <div ref={themeEditorRef} className="mt-8 animate-fade-in">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <button onClick={() => setEditingThemeIndex(null)} className="text-muted-foreground hover:text-white">&times; Close Editor</button>
                                </div>
                                <ThemeEditor 
                            ...
                            <div className="space-y-6">
                                {editedContent.socials.map((social) => !social.deletedOn &&(
                                    ...
                                    >
                                        <button
                                            onClick={() => {
                                                if (window.confirm(`Are you sure you want to move "${social.name}" to the trash? This is a temporary action and can be undone from the trash.`)) {
                                                    moveItemToTrash('social', { itemId: social.id });
                                                }
                                            }}
                ...
                <div className="animate-fade-in">
                    ...
                    {editedContent.services.map(service => !service.deletedOn && (
                        ...
                        >
                            <button
                                onClick={() => {
                                    if (window.confirm(`Are you sure you want to move the "${service.title}" service to the trash? This is a temporary action and can be undone from the trash.`)) {
                                        moveItemToTrash('service', { itemId: service.id });
                                    }
                                }}
                                        ...
                                        >
                                            ...
                                            <button
                                                onClick={(e) => {
                                                    ...
                                                    if(window.confirm('Are you sure you want to move this image to the trash? You can restore it later.')) {
                                                        moveItemToTrash('galleryImage', { serviceId: service.id, imageIndex: index });
                                                    }
                                                }}
